import type { CarbSourceOption, ElectrolyteSourceOption, ElectrolyteType } from '@/types/gelCalculator';

export const glucoseSourceOptions: CarbSourceOption[] = [
...
export const fructoseSourceOptions: CarbSourceOption[] = [
...
export const allCarbSourceOptions: CarbSourceOption[] = [...glucoseSourceOptions, ...fructoseSourceOptions];
export const sourceDataMap = new Map(allCarbSourceOptions.map(opt => [opt.label, opt]));

export const electrolyteSourceOptions: ElectrolyteSourceOption[] = [
...
export const SWEAT_RATES: number[] = [0.25, 0.75, 1.25, 1.75, 2.25, 2.75];
export const ELECTROLYTE_CONCENTRATIONS: Record<ElectrolyteType, number[]> = { Sodium: [10, 30, 50, 70, 90, 110], Chloride: [10, 30, 50, 70, 90, 110], Potassium: [1, 3, 5, 7, 9, 11], Magnesium: [0.1, 0.3, 0.5, 0.7, 0.9, 1.1], Calcium: [0.2, 0.6, 1.0, 1.4, 1.8, 2.2] }; // mmol/L
export const CONVERSION_FACTORS: Record<ElectrolyteType, number> = { Sodium: 23, Chloride: 35.5, Potassium: 39.1, Magnesium: 24.3, Calcium: 40.1 };
export const SWEAT_RATE_DESCRIPTIONS: string[] = ["Light sweating: Skin feels slightly damp", "Moderate sweating: Noticeable moisture on skin, shirt may show small damp patches", "Active sweating: Clothing shows distinct wet patches, sweat begins to drip occasionally", "Heavy sweating: Clothing widely soaked, continuous dripping of sweat", "Very heavy sweating: Clothes completely saturated, sweat constantly dripping/running", "Extreme sweating: Excessive sweating with clothes completely drenched"];
export const SALTINESS_DESCRIPTIONS: string[] = ["Very low salt content: Sweat barely tastes salty, minimal salt marks on clothing", "Low salt content: Slight salty taste, faint white marks may appear on dark clothing", "Moderate salt content: Clearly salty taste, visible white marks on dried clothing", "High salt content: Strong salty taste, prominent white marks after drying", "Very high salt content: Very strong salty taste, thick white residue on clothing", "Extreme salt content: Extremely salty, heavy white crystallization on skin and clothing"];

export const initialActiveElectrolytes: Record<ElectrolyteType, boolean> = { Sodium: true, Chloride: true, Potassium: true, Magnesium: true, Calcium: true };
export const initialManualTargets: Record<ElectrolyteType, number> = { Sodium: 0, Chloride: 0, Potassium: 0, Magnesium: 0, Calcium: 0 };
